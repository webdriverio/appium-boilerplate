...
describe('auth form', () => {
    it('should deny access with wrong credentials',  async () => {
        ...
        await FormPage.flash.waitForDisplayed({ timeout: 3000, timeoutMsg: 'Flash message not visible after invalid login' });
    ...
    it('should allow access with correct credentials',  async () => {
        ...
        await FormPage.flash.waitForDisplayed({ timeout: 3000, timeoutMsg: 'Flash message not visible after valid login' });
        await expect(await FormPage.flash).toHaveText(
